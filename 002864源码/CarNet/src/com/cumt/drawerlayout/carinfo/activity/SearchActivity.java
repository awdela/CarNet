...
public class SearchActivity extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		weizhangIntent.putExtra("appId",1);// 您的appId
		weizhangIntent.putExtra("appKey", "92c035ef9f63389767e913187df797e9");// 您的appKey
		...
		btnMycar.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				if(zxingBeanList == null){
					Toast.makeText(SearchActivity.this, "您还没有维护的车辆", Toast.LENGTH_SHORT).show();
					return;
				}
				showMyCarMsg();
			}
	...
	private boolean checkQueryItem(CarInfo car) {
		if (car.getCity_id() == 0) {
			Toast.makeText(SearchActivity.this, "请选择查询地", 0).show();
			return false;
		}

		if (car.getChepai_no().length() != 7) {
			Toast.makeText(SearchActivity.this, "您输入的车牌号有误", Toast.LENGTH_SHORT).show();
			return false;
		}
		...
		if (car.getCity_id() > 0) {
			...
			if (classno > 0) {
				if (car.getChejia_no().equals("")) {
					Toast.makeText(SearchActivity.this, "输入车架号不为空", 0).show();
					return false;
				}

				if (car.getChejia_no().length() != classno) {
					Toast.makeText(SearchActivity.this, "输入车架号后" + classno + "位",
							0).show();
					return false;
				}
			} else if (classno < 0) {
				if (car.getChejia_no().length() == 0) {
					Toast.makeText(SearchActivity.this, "输入全部车架号", 0).show();
					return false;
				}
			}
			...
			if (engineno > 0) {
				if (car.getEngine_no().equals("")) {
					Toast.makeText(SearchActivity.this, "输入发动机号不为空", 0).show();
					return false;
				}
				...
				if (car.getEngine_no().length() == 0) {
					Toast.makeText(SearchActivity.this, "输入全部发动机号", 0).show();
					return false;
				}
			}
			...
			if (registno > 0) {
				if (car.getRegister_no().equals("")) {
					Toast.makeText(SearchActivity.this, "输入证书编号不为空", 0).show();
					return false;
				}
				...
				if (car.getRegister_no().length() == 0) {
					Toast.makeText(SearchActivity.this, "输入全部证书编号", 0).show();
					return false;
				}
			}
			return true;
	...
	private void showMyCarMsg(){
		ItemDialog dialog = new ItemDialog(SearchActivity.this,"我的汽车", carNumList);
		dialog.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> parent, View view,
				...
				chejia_number.setText(bean.getVernum());//车架号
				engine_number.setText(bean.getEnginenum());//发动机号
			}
		});
		dialog.show();
	}
